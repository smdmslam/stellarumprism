...
**Specification:** Specced 2026-05-14 in `04.ADVANCED-AESTHETICS-AND-TYPOGRAPHY-SPEC.md` (Code block elevation, admonition callouts, midnight palette).
...
- **11.5 Dual-Mechanism Habit & Learning Module (Specced 2026-05-14):**
  - **Status:** DONE (shipped across frontend & backend)
  - **Requirement:** Implement explicit `/remember <rule>` slash command and implicit `[HABIT SUGGESTION]` stream interception toast card.
  - **Storage:** Stores atomic JSON rules in `~/.config/prism/habits.json` and injects them into system prompts.
  - **Why:** Combines explicit user direction with autonomous habit sensing for low-friction personalization.
...
- **12.3 IP Isolation & History Sanitization:**
  ...
  - **Goal**: Ensure a clean "Shipping Branch" or "Sale Repo" exists that contains zero proprietary musings or `prism/` workspace history.
...
| **11** Prism Habits | 3 | 3/3 DONE | Adaptive behavioral coaching & interaction mining (shipped across frontend & backend). |
...
- `stellarumdev/fixes/04.HABIT-AND-LEARNING-MODULE-IMPLEMENTATION.md` — Habit & Learning Module spec
- `stellarumdev/fixes/04.ADVANCED-AESTHETICS-AND-TYPOGRAPHY-SPEC.md` — Advanced Aesthetics & Typography spec
