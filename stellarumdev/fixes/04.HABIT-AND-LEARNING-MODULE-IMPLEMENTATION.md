...
### 3.3 User Configuration (`~/.config/prism/config.toml`)
In `src-tauri/src/config.rs`, expose a user toggle to allow disabling background habit mining:
```toml
[habits]
auto_habit_suggestions = true # Toggle for background Flash-Lite habit discovery
```
